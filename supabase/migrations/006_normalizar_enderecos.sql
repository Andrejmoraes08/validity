-- 006 (v2): Normalização de endereços para o padrão "Rua - Prédio - Nível - Apto"
-- Corrige endereços incompletos ("6 - 53 - 4 -" → "6 - 53 - 4 - 0") e
-- remove registros duplicados criados pela diferença de formato.
-- v2: lógica dentro de um bloco DO único — compatível com o SQL Editor do Supabase
-- (que executa cada comando em conexões diferentes). Seguro para reexecutar.

-- Função de normalização: 4 segmentos, segmento vazio vira 0
create or replace function normalizar_endereco(e text) returns text as $fn$
declare
  partes text[];
  resultado text[] := '{}';
  p text;
...
begin
  ...
  -- Tabela temporária com os ids a remover (existe só durante esta execução)
  create temp table _dup_remover (id uuid) on commit drop;

  -- ───────────────────────────────────────────────────────────
  -- 2) Duplicados por (sku, endereco_frac) entre itens ativos:
  --    copia saldo/validade do registro mais recente para o mantido
  -- ───────────────────────────────────────────────────────────
  with ranked as (
